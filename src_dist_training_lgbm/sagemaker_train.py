"""
SageMaker entry point for LightGBM training (single-node or multi-node distributed).
...
from train_lightgbm_dist import run_training
...
def _get_sagemaker_hosts():
    """Return (hosts, current_host) from SageMaker env."""
...
def main():
    ...
    # LightGBM pip package does NOT include GPU support (requires OpenCL build).
    # Always use CPU mode — LightGBM is already very fast on CPU.
    use_gpu = False
    if num_gpus > 0:
        print(f"NOTE: GPU detected but LightGBM pip uses CPU only (no OpenCL).")
        print(f"      Training will use all {num_cpus} CPU cores instead.")

    # Auto-detect num_workers: 1 worker per host (each uses all node CPUs)
    ...
    print(f"GPUs/node:  {num_gpus} (unused - LightGBM CPU mode)")
    print(f"CPUs/node:  {num_cpus}")
